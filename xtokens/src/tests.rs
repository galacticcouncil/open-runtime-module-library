...
fn transfer_and_swap_should_send_remote_swap_and_deposit_to_origin_with_swap_chain_reserve() {
	...
	ParaA::execute_with(|| {
		let fee = 40;
		assert_eq!(ParaTokens::free_balance(CurrencyId::B1, &BOB), 100 - fee);
	});
}
...
fn transfer_and_swap_should_send_remote_swap_and_deposit_to_origin_with_third_chain_reserve() {
	...
	ParaA::execute_with(|| {
		let fees = 80;
		assert_eq!(ParaTokens::free_balance(CurrencyId::D, &BOB), 500 - fees);
	});
}
...
fn transfer_and_swap_should_send_remote_swap_and_deposit_to_third_chain_with_swap_chain_reserve() {
	...
	ParaD::execute_with(|| {
		let fee = 40;
		assert_eq!(ParaTokens::free_balance(CurrencyId::B1, &BOB), 500 - fee);
	});
}
...
#[test]
fn transfer_and_swap_should_fail_when_zero_amount_sent() {
	TestNet::reset();
